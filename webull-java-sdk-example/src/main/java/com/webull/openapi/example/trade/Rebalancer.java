...
import com.webull.openapi.http.HttpApiClient.Action;
...
public class Rebalancer {
    ...
    public static void main(String[] args) {
        ...
        try {
            TradeApiService apiService = getApiService();
            ...
            for (Account account : accounts) {
                ...
                // System.out.println("desiredAcct: " + desiredAcct);
                ...
                if (acctNum.equals(desiredAcct)) {
                    ...
                    System.out.println("For account: " + acctNum + "(" + Acct.GetFriendlyName(acctNum) + ")");

                    List<HoldingAction> actions = GetDesiredPositionChanges();

                    // Add new positions as holdings
                    // holdings = AddSelectedPositions(holdings, actions);
                    numPositions += CountAddedPositions(actions);

                    // Remove holdings that are being closed
                    RemoveSelectedPositions(holdings, actions);
                    ...
                    // Get holding actions
                    holdingActions = DetermineHoldingActions(holdings, positionSize, actions, apiService);
                }
            }
    ...
    private static TradeApiService getApiService() {
    ...
    private static void RemoveSelectedPositions(List<HoldingInfo> holdings, List<HoldingAction> actions) {
        String ticker;

        for (HoldingAction remHldAction : actions) {
            if (!remHldAction.isSell()) {
                continue; // Skip if not a sell action
            }

            ticker = remHldAction.getTicker();

            // Collect items to remove first (safest approach)
            List<HoldingInfo> toRemove = new ArrayList<>();

            for (HoldingInfo remInfo : holdings) {
                if (ticker.equals(remInfo.getSymbol())) {
                    toRemove.add(remInfo);
                }
            }

            // Now safely remove them after the inner loop
            holdings.removeAll(toRemove);
        }
    }

    private static int CountAddedPositions(List<HoldingAction> actions) {
        int count = 0;
        String ticker = "";

        for (HoldingAction action : actions) {
    ...
    private static String GetDesiredAccount(List<Account> accts) {
        ...
        //System.out.println("acctID value = " + acctNum);
        if (acctId.isEmpty()) {
            System.out.println("Setting default to the first account");
            acctNum = accountList.get(0).getAccountNumber();
        }
    ...
    private static List<HoldingAction> DetermineHoldingActions(List<HoldingInfo> holdings, double positionSize,
            List<HoldingAction> additions, TradeApiService apiService) {
        // Use to get a list of all holding actions
        List<HoldingAction> actionsList = new ArrayList<>();
        List<HoldingAction> remActionsList = new ArrayList<>();
        HoldingAction action;
        double shareChange = 0, curPrice = 0.0, addCount;


        for (HoldingAction add : additions) {
            String ticker = add.getTicker();
            HoldingAction act;
            double shareCount;
            ...
            if (add.isBuy()) {
                ...
                try {
                    curPrice = Utils.getCurrentPrice(ticker);
                } catch (Exception e) {
                    // TODO: remove ticker because it's not valid - No, this is too late. If I removed here, position sizing would be off. 
                }

                // determine number of shares to buy
                shareCount = Utils.RoundDown(positionSize / curPrice, 0);

                // set shares to buy
                act = new HoldingAction(ticker, shareCount, ActionType.BUY);

                actionsList.add(act);
            }
            else if (add.isSell()) {
                System.out.println("REMOVING: " + ticker);
                //determine num shares owned
                shareCount = 999; //TODO: make this work: for now this could symbolize selling all
                act = new HoldingAction(ticker, shareCount, ActionType.SELL);
                actionsList.add(act);
            }
        }
        ...
        for (HoldingInfo holding : holdings) {
            ...
            //See if the holding is in the share list
            for (HoldingAction act : actionsList) {
                double posSize;
                if (act.getTicker().equals(ticker)) {
                    posSize = 0;
                }
                else {
                ...
                shareChange = Holdings.DetermineHoldingAction(holding, posSize);
            }
     ...
     */
    private static List<HoldingAction> GetDesiredPositionChanges() {
        ...
        while (true) {
            ...
            try {
                Utils.getCurrentPrice(ticker);
                // TODO: Add logic to validate this is a valid position to remove

                // // Here you will calculate how many shares to sell based on your rebalancing
                // logic
                // double sharesToSell = calculateSharesToSell(ticker, currentHoldings,
                // totalPortfolioValue);

                // if (sharesToSell > 0) {
                // actions.add(new HoldingAction(ticker, sharesToSell,
                // HoldingAction.ActionType.SELL));
    ...
    public void executeRebalanceTrade(TradeApiService apiService,
        ...
        try {
            ...
            try {
                instrumentId = apiService.getTradeInstrument(action.getTicker()).getInstrumentId(); // .getInstrument(action.getTicker());
                                                                                                    // // single ticker
            ...
            // PlaceOrderResponse response = apiService.placeStockOrder(accountId,
            // stockOrder);
